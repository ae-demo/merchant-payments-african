...
7. As a merchant, I want to see an in-app notification when a payment is
received, so that I know to fulfill the order.
...
- **Notifications**: payment-received notifications to merchants are
in-app only (a notification/alert visible in the merchant dashboard), not
SMS or email. Customers see an on-screen confirmation after paying and
receive no email or SMS receipt, since no customer account or contact
detail is collected.
